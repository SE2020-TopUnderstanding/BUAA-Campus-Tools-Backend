...
class UserFeedback(APIView):
    ...
    def get(request):
        ...
        for i in result:
            try:
                name = Student.objects.get(id=req['student_id']).name
    ...
    def post(request):
        ...
        参数:学生id，插入日期，反馈类别，具体内容
        :param request:
        :return:
        """
        req = request.data
        if (len(req) != 4) | (("student_id" not in req) | ("date" not in req) |
        ...
        Feedback(student_id=student, date=req["date"],
                 kind=req["kind"], content=req["content"]).save()
